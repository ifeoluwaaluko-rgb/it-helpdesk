...
from .services import get_configs
...
def save_smtp(request):
    ...
    cfg, _ = IntegrationConfig.objects.get_or_create(integration='email_smtp', defaults={'use_ssl': False})
...
def save_imap(request):
    if not _require_manager(request):
        return redirect('settings_home')
    cfg, _ = IntegrationConfig.objects.get_or_create(integration='email_imap', defaults={'use_ssl': True})
    cfg.host = request.POST.get('host', '').strip()
    cfg.port = int(request.POST.get('port', 993) or 993)
    cfg.username = request.POST.get('username', '').strip()
    cfg.use_ssl = request.POST.get('use_ssl', 'on') == 'on'
    cfg.is_active = request.POST.get('is_active') == 'on'
    pw = request.POST.get('password', '').strip()
    if pw:
        cfg.password = pw
...
def save_graph(request):
    if not _require_manager(request):
        return redirect('settings_home')
    cfg, _ = IntegrationConfig.objects.get_or_create(integration='microsoft_graph', defaults={'use_ssl': False})
...
def save_whatsapp(request):
    if not _require_manager(request):
        return redirect('settings_home')
    cfg, _ = IntegrationConfig.objects.get_or_create(integration='whatsapp', defaults={'use_ssl': False})
...
def save_teams(request):
    if not _require_manager(request):
        return redirect('settings_home')
    cfg, _ = IntegrationConfig.objects.get_or_create(integration='teams', defaults={'use_ssl': False})
...
def save_slack(request):
    if not _require_manager(request):
        return redirect('settings_home')
    cfg, _ = IntegrationConfig.objects.get_or_create(integration='slack', defaults={'use_ssl': False})
    cfg.webhook_url = request.POST.get('webhook_url', '').strip()
    cfg.is_active = request.POST.get('is_active') == 'on'
    cfg.updated_by = request.user
    cfg.save()
...
def save_generic_webhook(request):
    if not _require_manager(request):
        return redirect('settings_home')
    cfg, _ = IntegrationConfig.objects.get_or_create(integration='generic_webhook', defaults={'use_ssl': False})
    cfg.host = request.POST.get('host', '').strip()
    cfg.webhook_url = request.POST.get('webhook_url', '').strip()
    cfg.is_active = request.POST.get('is_active') == 'on'
    token = request.POST.get('access_token', '').strip()
...
def save_openai(request):
    if not _require_manager(request):
        return redirect('settings_home')
    cfg, _ = IntegrationConfig.objects.get_or_create(integration='openai', defaults={'use_ssl': False})
...
def _test_smtp(cfg):
    return False, 'Outbound SMTP is disabled for this MVP. Inbound IMAP remains available.'


def _test_imap(cfg):
    try:
        import imaplib
        mail = imaplib.IMAP4_SSL(cfg.host, cfg.port or 993) if getattr(cfg, 'use_ssl', True) else imaplib.IMAP4(cfg.host, cfg.port or 143)
...
def _test_webhook(cfg):
    try:
        import urllib.request, json
        payload = json.dumps({'text': 'Zynaros webhook test: connection OK'}).encode()
